...
k.body.removeChild(a);return b!==H&&0<b.length&&"none"!==b}function Ua(){if(Q||R){var a=Fa();c(".imageEditor-wrapper").off("touchstart "+a.down).on("touchstart "+a.down,La);c(".imageEditor-wrapper").off("touchmove "+a.move).on("touchmove "+a.move,Ka)}}function Va(){if(Q||R){var a=Fa();c(".imageEditor-wrapper").off("touchstart "+a.down);c(".imageEditor-wrapper").off("touchmove "+a.move)}}function Fa(){return l.PointerEvent?{down:"pointerdown",move:"pointermove"}:{down:"MSPointerDown",move:"MSPointerMove"}}function la(a){var b=
...
position:"relative"}),f.addClass("imageEditor-wrapper"),c("html").addClass("fp-enabled")):T("error","Error! Fullpage.js needs to be initialized with a selector. For example: $('#myContainer').imageEditor();");d.css3&&(d.css3=Ta());e.setAllowScrolling(!0);f.removeClass("fp-destroyed");c(d.sectionSelector).each(function(){c(this).addClass("fp-section")});c(d.slideSelector).each(function(){c(this).addClass("fp-slide")});d.navigation&&Ja();c(".fp-section").each(function(a){var b=c(this),e=c(this).find(".fp-slide"),
